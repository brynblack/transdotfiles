{ lib, stdenv, buildNpmPackage, fetchFromGitHub, electron_36, dart-sass, pnpm_10
, darwin, copyDesktopItems, makeDesktopItem, }:
let
  pname = "feishin";
  version = "0.21.2";
...
in buildNpmPackage {
  ...
  '' + ''
    npm exec electron-builder -- \
      --dir \
      -c.electronDist=${
        if stdenv.hostPlatform.isDarwin then "./" else electron.dist
      } \
  ...
  meta = {
    ...
    mainProgram = "feishin";
    maintainers = with lib.maintainers; [ onny jlbribeiro ];
  };
}
